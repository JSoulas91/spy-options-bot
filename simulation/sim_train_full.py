...
def simulate_trade(day_idx: int, step_idx: int, prices: list[float], volumes: list[int], vix: float):
    minute_cutoff = 60 * 5 * 10  # 10 bars of 1h (300m) = 10h = 600min
    ...
    start_idx = RNG.randint(minute_cutoff, len(prices) - 31)
    ...
    action = meta_agent.act(meta_state)
    final_price = prices[start_idx + 30]
...
def main():
    ...
    for day in range(SIM_DAYS):
        ...
        volumes = [RNG.randint(300000, 1000000) for _ in prices]
        vix = RNG.uniform(14, 28)

        trades = []
        for trade_idx in range(TRADES_PER_DAY):
            log_entry = simulate_trade(day, trade_idx, prices, volumes, vix)
            if log_entry:
